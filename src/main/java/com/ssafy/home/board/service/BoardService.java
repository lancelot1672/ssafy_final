...
public class BoardService {
	...
	public Map<String, Object> makePage2(int page, List<String> guguns) { // 현재 페이지
		/// 총게시글 수 구하기
		int totalCount = dao.getTotalCount2(guguns);
		int totalPage = totalCount / COUNT_PER_PAGE; // 55 / 10 = 5
		if (totalCount % COUNT_PER_PAGE > 0) // 55 % 10 = 3이면 페이지 하나 더 필요함
			totalPage++;
		///
		...
		if (endPage > totalPage)
			endPage = totalPage;
		int startRow = (page - 1) * COUNT_PER_PAGE; // (1-1)*10=0, (2-1)* 10=10, (3-1)*10=20
		List<BoardDTO> boardList = dao.selectList2(startRow, COUNT_PER_PAGE, guguns);
		//////////////// 페이지에 보여줄 모든 데이터 확보 완료//////////////////////////
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("startPage", startPage);
		result.put("endPage", endPage);
